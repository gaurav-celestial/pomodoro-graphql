...
import useFetchTasksOfUser from "../hooks/useFetchTasksOfUser.js";

function Homepage({ handleCurrentTask }) {
  const navigate = useNavigate();
  const user = useSelector((state) => state.settings.user);

  const { data, error, loading, fetchTasksHandler } = useFetchTasksOfUser();
  ...
  console.log(user);
  useEffect(() => {
    fetchTasksHandler(user.id);
  }, []);

  useEffect(() => {
    if (data?.getTaskOfUser) {
      setTaskGroups(
        data.getTaskOfUser.map((task) => {
          return { ...task, tasks: JSON.parse(task.tasks) };
        })
      );
    }
  }, [data]);
