...
const Form = ({ onRemove, tasksSubmit, setActiveTabToDefault }) => {
  ...
  const { data, error, loading, createTaskHandler } = useCreateTask();
  ...
  const submitHandler = (e) => {
    ...
    async function updateTasks() {
      console.log(createTaskHandler(allTasks));
    }
    updateTasks();
  };
  ...
  useEffect(() => {
    if (data) {
      console.log(data?.createTaskGroup, loading, error);
      tasksSubmit({
        ...data.createTaskGroup,
        tasks: JSON.parse(data.createTaskGroup.tasks),
      });
      onRemove();
